...
from app.tiles import tile_store, interaction_store
...
async def move_tile(tile_id: int, body: MoveRequest):
...
async def interact_tile(tile_id: int, body: InteractionRequest):
...
async def reset_tile_interaction(tile_id: int):
    await interaction_store.reset_one(tile_id)
    # Also remove the tile if it's a user-question tile (resolved + User Question source)
    tile_removed = False
