-- V007: Knowledge Graph schema for entity and relationship storage
...
CREATE TABLE IF NOT EXISTS graph.entity_types (
...
CREATE TABLE IF NOT EXISTS graph.relationship_types (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name VARCHAR(100) NOT NULL UNIQUE,
    description TEXT,
...
CREATE INDEX IF NOT EXISTS idx_entities_kb ON graph.entities(knowledge_base_id);
CREATE INDEX IF NOT EXISTS idx_entities_type ON graph.entities(entity_type);
CREATE INDEX IF NOT EXISTS idx_entities_canonical ON graph.entities(canonical_name);
CREATE INDEX IF NOT EXISTS idx_entities_name_trgm ON graph.entities USING gin (name gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS idx_relationships_kb ON graph.relationships(knowledge_base_id);
CREATE INDEX IF NOT EXISTS idx_relationships_source ON graph.relationships(source_entity_id);
CREATE INDEX IF NOT EXISTS idx_relationships_target ON graph.relationships(target_entity_id);
CREATE INDEX IF NOT EXISTS idx_relationships_type ON graph.relationships(relationship_type);

-- Entity mentions in chunks (for provenance tracking)
CREATE TABLE IF NOT EXISTS graph.entity_mentions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_entity_mentions_entity ON graph.entity_mentions(entity_id);
CREATE INDEX IF NOT EXISTS idx_entity_mentions_chunk ON graph.entity_mentions(chunk_id);

-- Graph queries cache (for expensive traversals)
CREATE TABLE IF NOT EXISTS graph.query_cache (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    knowledge_base_id UUID REFERENCES rag.knowledge_bases(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_query_cache_expires ON graph.query_cache(expires_at);

-- Useful views

-- Entity statistics per knowledge base
CREATE OR REPLACE VIEW graph.entity_stats AS
SELECT
...
CREATE OR REPLACE VIEW graph.relationship_stats AS
SELECT
    kb.id as knowledge_base_id,
    kb.name as knowledge_base_name,
    r.relationship_type,
