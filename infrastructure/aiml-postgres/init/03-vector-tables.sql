...
CREATE TABLE vectors.collections (
...
CREATE INDEX ON vectors.embeddings USING hnsw (embedding vector_cosine_ops);

-- Index for collection queries
CREATE INDEX idx_embeddings_collection ON vectors.embeddings(collection_id);
CREATE INDEX idx_embeddings_content_hash ON vectors.embeddings(content_hash);

-- Embeddings with different dimensions (for other models)
-- Example: 384 for all-MiniLM-L6-v2, 768 for BERT, 4096 for larger models

CREATE TABLE vectors.embeddings_384 (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    collection_id UUID NOT NULL REFERENCES vectors.collections(id) ON DELETE CASCADE,
...
CREATE INDEX ON vectors.embeddings_384 USING hnsw (embedding vector_cosine_ops);
CREATE INDEX idx_embeddings_384_collection ON vectors.embeddings_384(collection_id);

CREATE TABLE vectors.embeddings_768 (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    collection_id UUID NOT NULL REFERENCES vectors.collections(id) ON DELETE CASCADE,
    content_hash VARCHAR(64),
    content TEXT,
