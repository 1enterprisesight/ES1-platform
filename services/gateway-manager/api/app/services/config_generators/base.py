...
class ConfigGenerator(ABC):
    ...
    def _get_rate_limit(self, settings: dict[str, Any]) -> int:
        """Get rate limit from settings with default."""
        return settings.get("rate_limit", 100)

    def _get_timeout(self, settings: dict[str, Any]) -> str:
        """Get timeout from settings with default."""
        return settings.get("timeout", "30s")
