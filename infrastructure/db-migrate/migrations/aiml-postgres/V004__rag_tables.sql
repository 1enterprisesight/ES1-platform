-- V004: RAG (Retrieval-Augmented Generation) tables
...
CREATE INDEX IF NOT EXISTS idx_documents_source ON rag.documents(source_id);
CREATE INDEX IF NOT EXISTS idx_documents_kb ON rag.documents(knowledge_base_id);
...
CREATE INDEX IF NOT EXISTS idx_chunks_embedding ON rag.chunks(embedding_id);
CREATE INDEX IF NOT EXISTS idx_chunks_content_hash ON rag.chunks(content_hash);

-- Link knowledge bases to sources
CREATE TABLE IF NOT EXISTS rag.knowledge_base_sources (
...
CREATE TABLE IF NOT EXISTS rag.search_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_search_history_kb ON rag.search_history(knowledge_base_id);
CREATE INDEX IF NOT EXISTS idx_search_history_created ON rag.search_history(created_at);
