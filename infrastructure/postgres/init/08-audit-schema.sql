-- Audit schema for full API visibility
...
CREATE INDEX idx_api_requests_user ON audit.api_requests(user_id);
CREATE INDEX idx_api_requests_path ON audit.api_requests(path);

-- Composite index for service-to-service queries
CREATE INDEX idx_api_requests_service_pair ON audit.api_requests(source_service, destination_service);

-- Partial index for errors only (faster error analysis)
CREATE INDEX idx_api_requests_errors ON audit.api_requests(timestamp, destination_service)
...
CREATE INDEX idx_service_deps_source ON audit.service_dependencies(source_service);
CREATE INDEX idx_service_deps_dest ON audit.service_dependencies(destination_service);
...
CREATE INDEX idx_security_events_unresolved ON audit.security_events(timestamp) WHERE NOT resolved;
...
CREATE UNIQUE INDEX idx_api_keys_hash ON audit.api_keys(key_hash);
CREATE INDEX idx_api_keys_prefix ON audit.api_keys(key_prefix);
CREATE INDEX idx_api_keys_service ON audit.api_keys(service_name);
...
CREATE VIEW audit.service_health AS
SELECT
