-- MLOps tables for model registry and deployment tracking
...
CREATE INDEX idx_models_name ON mlops.models(name);
CREATE INDEX idx_models_type ON mlops.models(model_type);

-- Model deployments
CREATE TABLE mlops.deployments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_deployments_model ON mlops.deployments(model_id);
CREATE INDEX idx_deployments_env ON mlops.deployments(environment);
CREATE INDEX idx_deployments_status ON mlops.deployments(status);

-- Inference logs for monitoring and debugging
CREATE TABLE mlops.inference_logs (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_inference_logs_deployment ON mlops.inference_logs(deployment_id);
CREATE INDEX idx_inference_logs_created ON mlops.inference_logs(created_at);
CREATE INDEX idx_inference_logs_status ON mlops.inference_logs(status);

-- Feature store (basic implementation, can use Feast for advanced features)
CREATE TABLE mlops.feature_groups (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_feature_values_group ON mlops.feature_values(feature_group_id);
CREATE INDEX idx_feature_values_entity ON mlops.feature_values(entity_id);
CREATE INDEX idx_feature_values_timestamp ON mlops.feature_values(event_timestamp);

-- Prompt templates and versioning
CREATE TABLE mlops.prompts (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name VARCHAR(255) NOT NULL,
...
CREATE INDEX idx_prompts_name ON mlops.prompts(name);
CREATE INDEX idx_prompts_active ON mlops.prompts(is_active);
