-- V005: Agent tables for conversation history, state management, and orchestration
...
CREATE INDEX IF NOT EXISTS idx_sessions_agent ON agents.sessions(agent_id);
CREATE INDEX IF NOT EXISTS idx_sessions_user ON agents.sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_sessions_status ON agents.sessions(status);
...
CREATE INDEX IF NOT EXISTS idx_messages_session ON agents.messages(session_id);
...
CREATE INDEX IF NOT EXISTS idx_memory_agent ON agents.memory(agent_id);
CREATE INDEX IF NOT EXISTS idx_memory_session ON agents.memory(session_id);
CREATE INDEX IF NOT EXISTS idx_memory_type ON agents.memory(memory_type);

-- Agent networks (crews, swarms, hierarchies)
CREATE TABLE IF NOT EXISTS agents.networks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name VARCHAR(255) NOT NULL UNIQUE,
    description TEXT,
...
-- Network membership
CREATE TABLE IF NOT EXISTS agents.network_members (
...
CREATE INDEX IF NOT EXISTS idx_executions_agent ON agents.executions(agent_id);
CREATE INDEX IF NOT EXISTS idx_executions_network ON agents.executions(network_id);
CREATE INDEX IF NOT EXISTS idx_executions_status ON agents.executions(status);
CREATE INDEX IF NOT EXISTS idx_executions_started ON agents.executions(started_at);
