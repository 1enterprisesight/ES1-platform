...
from app.models import Exposure, Deployment, ConfigVersion, ExposureChange, EventLog
from app.services.config_generators import GeneratorRegistry
...
class DeploymentEngine:
    ...
    def _create_backend(self) -> DeploymentBackend:
        ...
        if RUNTIME_MODE == RuntimeMode.KUBERNETES:
            from app.services.deployment_kubernetes import KubernetesDeploymentEngine
            return KubernetesDeploymentEngine()
        else:
            from app.services.deployment_docker import DockerDeploymentEngine
    ...
    async def deploy_changes(
        ...
        try:
            # 1. Get all approved changes
            approved_query = select(ExposureChange).where(ExposureChange.status == "approved")
            approved_result = await db.execute(approved_query)
            changes = list(approved_result.scalars().all())

            if not changes:
                return (False, "No approved changes to deploy", None)

            # 2. Get current active exposures (deployed and not removed)
            current_query = (
                select(Exposure)
            ...
            exposures_to_deploy = list(current_result.scalars().all())

            # 3. Apply changes to exposure list
            for change in changes:
                if change.change_type == "add":
                    if change.exposure_id:
                        existing_exposure = await db.get(Exposure, change.exposure_id)
                        if existing_exposure:
                            exposures_to_deploy.append(existing_exposure)
                            continue
                    ...
                    db.add(new_exposure)
                    await db.flush()
                    exposures_to_deploy.append(new_exposure)
                    change.exposure_id = new_exposure.id

                elif change.change_type == "remove":
                    exposures_to_deploy = [
                        e for e in exposures_to_deploy if e.id != change.exposure_id
                    ]

                elif change.change_type == "modify":
                    for exp in exposures_to_deploy:
            ...
            config = await self.generate_complete_config(exposures_to_deploy, db)

            # 5. Get next version
            next_version = await self._get_next_version(db)

            # 6. Save configuration version
            config_version = ConfigVersion(
                version=next_version,
                config_snapshot=config,
                created_by=deployed_by,
                commit_message=commit_message or f"Deploy {len(changes)} changes",
            )
            db.add(config_version)
            await db.flush()
            ...
            # 9. Trigger rolling restart (if applicable)
            if hasattr(self._backend, 'rolling_restart'):
                if not await self._backend.rolling_restart():
                    deployment.status = "failed"
                    deployment.error_message = "Failed to restart deployment"
            ...
            # 12. Mark deployment as successful
            deployment.status = "succeeded"
            deployment.health_check_passed = True
            deployment.completed_at = datetime.utcnow()

            # 13. Update exposures and changes
            for change in changes:
                if change.change_type == "add":
                    ...
                    exposure = await db.get(Exposure, change.exposure_id)
                    if exposure:
                        exposure.removed_in_version_id = config_version.id
                ...
                change.deployed_in_version_id = config_version.id
                change.deployed_at = datetime.utcnow()

            # 14. Mark old config versions as inactive
            old_versions_query = select(ConfigVersion).where(ConfigVersion.is_active == True)
            old_versions_result = await db.execute(old_versions_query)
            for old_version in old_versions_result.scalars().all():
                old_version.is_active = False

            # 15. Mark new config version as active
            config_version.is_active = True
            config_version.deployed_to_gateway_at = datetime.utcnow()

            # 16. Log deployment event
            event = EventLog(
                event_type="deployment_succeeded",
                entity_type="deployment",
                entity_id=deployment.id,
            ...
            return (
                True,
                f"Successfully deployed {len(changes)} changes",
                deployment_id,
            )
